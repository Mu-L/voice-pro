...
class WhisperTimestampedInference:
    def __init__(self):
    ...
    def available_models():
        return whisper_timestamped.available_models()
    ...
    def transcribe_live(self,
        ...
        try:
            ...
            logger.error(f"[abus_asr_whisper_timestamped.py] transcribe_live - An error occurred: {e}")
            return None, None
        finally:
    ...
    def transcribe(self,
        ...
        if params.lang == "Automatic Detection":
            ...
            language_code_dict = {value: key for key, value in whisper_timestamped.tokenizer.LANGUAGES.items()}
    ...
    def _denoise_live(self, input: np.ndarray, denoise_level: int = 0) -> np.ndarray:
        logger.debug(f'[abus_asr_whisper_timestamped.py] _denoise_live:input.shape = {input.shape}') 
